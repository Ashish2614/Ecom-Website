...
export default function AdminSubCategoryUpdatePage() {
  ...
  async function postData(e) {
    ...
    if (error)
      ...
      let item = SubcategoryStateData.find(trash => trash.id !== id && trash.name?.toLocaleLowerCase() === data.name?.toLocaleLowerCase())
      if (item) {
        setErrorMessage({ ...errorMessage, name: "Subcategory with this name Already Exist" })
        setShow(true)
        return
      }
      dispatchEvent(updateSubCategory({ ...data }))
      ...
      // dispatchEvent(createSubCategory(formData))
