...
export default function AdminFeatureUpdatePage() {
  ...
  async function postData(e) {
    ...
    if (error)
      ...
      let item = FeatureStateData.find(trash => trash.id !== id && trash.name?.toLocaleLowerCase() === data.name?.toLocaleLowerCase())
      if (item) {
        setErrorMessage({ ...errorMessage, name: "Feature with this name Already Exist" })
        setShow(true)
        return
      }
      dispatchEvent(updateFeature({ ...data }))
            ...
            <form onSubmit={postData}>
              <div className="row">
                ...
                <div className="col-12 mb-3">
                  ...
                  <input type="text" name="shortDescription" value={data.shortDescription} rows={3} onChange={getInputData} placeholder='Feature Name' className={`form-control ${show && errorMessage.name ? 'border-danger' : 'border-primary'}`} /> {show && errorMessage.shortDescription ? <p className='text-danger text-capitalize'>{errorMessage.name}</p> : null}
                </div>

                <div className="col-md-6 mb-3">
                  <label>Icon*</label>
                  <input type="text" name="icon" value={data.icon} onChange={getInputData} className={`form-control ${show && errorMessage.icon ? 'border-danger' : 'border-primary'}`} placeholder="Booststrap Icon Tag like  <i class = 'bi bi-list'></i>" />
