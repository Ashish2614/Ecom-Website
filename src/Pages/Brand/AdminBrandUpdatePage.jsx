...
export default function AdminBrandUpdatePage() {
  ...
  async function postData(e) {
    ...
    if (error)
      ...
      let item = BrandStateData.find(trash => trash.id !== id && trash.name?.toLocaleLowerCase() === data.name?.toLocaleLowerCase())
      if (item) {
        setErrorMessage({ ...errorMessage, name: "Brand with this name Already Exist" })
        setShow(true)
        return
      }
      dispatchEvent(updateBrand({ ...data }))
      ...
      // dispatchEvent(createBrand(formData))
